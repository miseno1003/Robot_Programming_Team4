...
# |오차| <= 이 값이면 '중앙', 그 밖이면 부호에 따라 왼쪽/오른쪽
...
class RedBellDetector:
    ...
    def position_label(found, error):
        """검출 결과를 사람이 읽기 좋은 위치 라벨로 변환

        반환: '없음' / '있음-왼쪽' / '있음-중앙' / '있음-오른쪽'
        error > 0 : 벨이 화면 오른쪽
        error < 0 : 벨이 화면 왼쪽
        """
        if not found:
...
# ----- 벨 탐색(미검출 시) 설정 -----
# 벨이 안 보일 때 왼쪽(반시계)으로 돌며 찾습니다.
# angular_z 양수 = 반시계 = 왼쪽
SEARCH_ANGULAR = 0.35       # 탐색 회전 속도 (rad/s)
SEARCH_STEP_TIME = 0.4      # 한 번 회전 명령 유지 시간 (s)
# 한 바퀴(약 360도)를 다 돌아도 못 찾으면 경고만 출력하고 계속 탐색
SEARCH_FULL_TURN = 2.0 * math.pi   # 360도(rad)
...
class BellController(Node):
    ...
    def align_to_bell(self, stage):
        ...
        # === 정렬 전 VLM으로 벨 위치 먼저 확인 (로그용) ===
        bgr_pre = self._get_bgr()
        if bgr_pre is not None:
            vlm_pos = self.vlm.bell_position(bgr_pre)
            self.get_logger().info(
                f'[{name}] VLM 벨 위치 판별: {vlm_pos}'
            )
        else:
            self.get_logger().warn(
                f'[{name}] VLM 위치 확인용 이미지 수신 실패'
            )
        ...
        vlm_checked = False
        search_accum = 0.0   # 탐색 회전 누적 각도(rad), 한 바퀴 감지용
        ...
        while rclpy.ok():
            ...
            # 매 프레임 벨 위치 라벨 로그 (없음 / 있음-왼쪽 / 중앙 / 오른쪽)
            ...
            if found:
                not_found_count = 0
                search_accum = 0.0  # 벨을 찾았으니 탐색 누적각 초기화
                ...
                if not vlm_checked:
                    ...
                    if exists:
                        self.get_logger().info(
                            f'[{name}] VLM: 벨 존재함. '
                            f'색 검출 임계값 확인 필요. 왼쪽 탐색 회전 진행.'
                        )
                    else:
                        self.get_logger().info(
                            f'[{name}] VLM: 벨 안 보임. 왼쪽으로 회전하며 탐색.'
                        )

                # 벨을 찾기 위해 왼쪽(반시계)으로 탐색 회전
                self.get_logger().info(
                    f'[{name}] 벨 탐색: 왼쪽으로 회전 '
                ...
                # 한 바퀴를 다 돌아도 못 찾으면 경고 후 누적각 초기화
                if search_accum >= SEARCH_FULL_TURN:
                    self.get_logger().warn(
                        f'[{name}] 한 바퀴(360도)를 돌았지만 벨을 못 찾음. '
                        f'다시 한 바퀴 탐색합니다. '
                        f'(빨강 검출 임계값/조명 확인 필요)'
                    )
                    search_accum = 0.0
